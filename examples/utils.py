...
def display_borderless_tables(img: Image, ocr: OCRInstance) -> np.ndarray:
    ...
    white_img = cv2.cvtColor(
        255 * np.ones((display_image.shape[0], width), dtype=np.uint8), cv2.COLOR_GRAY2RGB
    )

    # Stack images
    return np.hstack([next(iter(img.images)).copy(), white_img, display_image])

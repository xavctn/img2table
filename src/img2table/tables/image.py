...
class TableImage(BaseModel):
    ...
    _median_line_sep: float | None = None
    _thresh: np.ndarray | None = None
    _contours: list[Cell] | None = None
    ...
    def model_post_init(self, context: Any, /) -> None:  # noqa: ARG002
        ...
        self._char_length, self._median_line_sep, self._contours = compute_img_metrics(
            thresh=self.thresh.copy()
        )

    @property
    def char_length(self) -> float:
        if self._char_length is None:
            raise ValueError("char_length is not initialized")
        return self._char_length

    @property
    def median_line_sep(self) -> float:
        if self._median_line_sep is None:
            raise ValueError("median_line_sep is not initialized")
        return self._median_line_sep

    @property
    def thresh(self) -> np.ndarray:
        if self._thresh is None:
            raise ValueError("thresh is not initialized")
        return self._thresh

    @property
    def contours(self) -> list[Cell]:
        if self._contours is None:
            raise ValueError("contours is not initialized")
        return self._contours

    @property
    def lines(self) -> list[Line]:
        if self._lines is None:
            raise ValueError("lines is not initialized")
        return self._lines

    @property
    def tables(self) -> list[Table]:
        if self._tables is None:
            raise ValueError("tables is not initialized")
        return self._tables
    ...
    def white_img(self) -> np.ndarray:
        ...
        for line in self.lines:
            if line.horizontal:
                cv2.rectangle(
                    white_img,
                    (line.x1 - line.thickness, line.y1),
                    (line.x2 + line.thickness, line.y2),
                    (255, 255, 255),
                    3 * line.thickness,
                )
            elif line.vertical:
                cv2.rectangle(
                    white_img,
                    (line.x1, line.y1 - line.thickness),
    ...
    def extract_bordered_tables(
        ...
        self._tables = get_tables(
            cells=cells, elements=self.contours, lines=self.lines, char_length=self.char_length
        )
        ...
        self._tables = merge_consecutive_tables(tables=self.tables, contours=self.contours)

        # Post filter bordered tables
        self._tables = [tb for tb in self._tables if min(tb.nb_rows, tb.nb_columns) >= 2]

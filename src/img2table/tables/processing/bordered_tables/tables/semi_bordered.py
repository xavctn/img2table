...
def get_lines_in_cluster(cluster: list[Cell], lines: list[Line]) -> tuple[list[Line], list[Line]]:
    ...
    v_lines_cl = [
        line
        for line in lines
        if line.vertical and min([abs(line.x1 - x) for x in x_values_cl]) <= 0.05 * (x_max - x_min)
    ]
...
def identify_table_dimensions(
    cluster: list[Cell], h_lines_cl: list[Line], v_lines_cl: list[Line], char_length: float
    ...
    if h_lines_cl:
        ...
        if len({h_lines_cl[0], h_lines_cl[-1]}.difference(set(left_end_lines))) == 0:
            left_val = (
                ...
                if min([c.x1 for c in cluster]) - left <= 2 * char_length
                else left
            )
        else:
        ...
        if len({h_lines_cl[0], h_lines_cl[-1]}.difference(set(right_end_lines))) == 0:
            right_val = (
                ...
                if right - max([c.x2 for c in cluster]) <= 2 * char_length
                else right
            )
        else:
    ...
    if v_lines_cl:
        ...
        if len({v_lines_cl[0], v_lines_cl[-1]}.difference(set(top_end_lines))) == 0:
            top_val = (
                ...
                if min([c.y1 for c in cluster]) - top <= 2 * char_length
                else top
            )
        else:
        ...
        # Bottom end
        bottom_end_lines = [
            line for line in v_lines_cl if bottom - line.y2 <= 0.05 * (bottom - top)
        ]
        if len({v_lines_cl[0], v_lines_cl[-1]}.difference(set(bottom_end_lines))) == 0:
            bottom_val = (
                max([c.y2 for c in cluster])
                if bottom - max([c.y2 for c in cluster]) <= 2 * char_length
                else bottom
            )
        else:
...
def identify_potential_new_cells(
    ...
    x_cluster = sorted(
        {c.x1 for c in cluster}.union({c.x2 for c in cluster}).union({left_val, right_val})
    )
    y_cluster = sorted(
        {c.y1 for c in cluster}.union({c.y2 for c in cluster}).union({top_val, bottom_val})
    )
    ...
    y_vals = sorted(
        {top_val, bottom_val}.union(
            {ln.y1 for ln in h_lines_cl if min(ln.x2, x2) - max(ln.x1, x1) >= 0.9 * (x2 - x1)}
        )
    )
    for y1, y2 in pairwise(y_vals):
    ...
    x_vals = sorted(
        {left_val, right_val}.union(
            {ln.x1 for ln in v_lines_cl if min(ln.y2, y2) - max(ln.y1, y1) >= 0.9 * (y2 - y1)}
        )
    )
    for x1, x2 in pairwise(x_vals):
        new_cell = Cell(x1=x1, y1=y1, x2=x2, y2=y2)
        if new_cell.area > 0:
            new_cells.append(new_cell)
...
def update_cluster_cells(cluster: list[Cell], new_cells: list[Cell]) -> list[Cell]:
    ...
    df_cluster = pl.DataFrame(
        [{"x1": c.x1, "y1": c.y1, "x2": c.x2, "y2": c.y2, "area": c.area} for c in cluster]
    )
    df_cells = pl.DataFrame(
        [
            {"idx": idx, "x1": c.x1, "y1": c.y1, "x2": c.x2, "y2": c.y2, "area": c.area}
    ...
    df_cells_indep = (
        df_cells.join(df_cluster, how="cross")
        .with_columns(
            x_overlap=pl.min_horizontal("x2", "x2_right") - pl.max_horizontal("x1", "x1_right"),
            y_overlap=pl.min_horizontal("y2", "y2_right") - pl.max_horizontal("y1", "y1_right"),
        )
        .with_columns(
            x_overlap=pl.max_horizontal(pl.lit(0), "x_overlap"),
    ...
    df_dups = (
        df_cells_indep.join(df_cells_indep, how="cross")
        .filter(pl.col("area") <= pl.col("area_right"), pl.col("idx") != pl.col("idx_right"))
        .with_columns(
            x_overlap=pl.min_horizontal("x2", "x2_right") - pl.max_horizontal("x1", "x1_right"),
            y_overlap=pl.min_horizontal("y2", "y2_right") - pl.max_horizontal("y1", "y1_right"),
        )
    ...
    df_final_cells = df_cells_indep.join(df_dups, on=["idx"], how="anti").select(
        "x1", "y1", "x2", "y2"
    )
...
def add_semi_bordered_cells(
    ...
    left_val, right_val, top_val, bottom_val = identify_table_dimensions(
        cluster=cluster, h_lines_cl=h_lines_cl, v_lines_cl=v_lines_cl, char_length=char_length
    )

...
def coherent_table(tb: Table, elements: list[Cell]) -> Table | None:
    """
    ...
    df_rows = pl.DataFrame(
        [
            {"row_id": row_id, "x1": c.x1, "y1": c.y1, "x2": c.x2, "y2": c.y2}
    ...
    rel_rows = (
        df_relevant_rows.join(df_elements, how="cross")
        .with_columns(
            ...
            area=(pl.col("x2_right") - pl.col("x1_right"))
            * (pl.col("y2_right") - pl.col("y1_right")),
        )
    ...
    if len(rel_rows) > 0:
        # Get new rows
        new_rows = tb.items[rel_rows[0].get("min_row") : rel_rows[0]["max_row"] + 1]
        if len(new_rows) >= 2:
...
def deduplicate_tables(identified_tables: list[Table], existing_tables: list[Table]) -> list[Table]:
    ...
    for table in identified_tables:
        if not any(
            max(
                is_contained_cell(inner_cell=table.cell, outer_cell=tb.cell, percentage=0.1),
...
def identify_borderless_tables(
    ...
    for table_segment in table_segments:
        ...
        column_group = identify_columns(table_segment=table_segment, char_length=char_length)

        if column_group:
            # Identify potential table rows
            row_delimiters = identify_delimiter_group_rows(
                column_group=column_group, contours=contours
            )

            if row_delimiters:
                # Create table from column group and rows
                borderless_table = identify_table(
                ...
                if borderless_table:
                    # Check table
                    corrected_table = coherent_table(
                        tb=borderless_table, elements=table_segment.elements
                    )

                    if corrected_table:
                        tables.append(corrected_table)

    return deduplicate_tables(identified_tables=tables, existing_tables=existing_tables)

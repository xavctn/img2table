...
def get_columns_delimiters(table_segment: TableSegment, char_length: float) -> list[Column]:
    ...
    for id_area, tb_area in enumerate(table_areas):
        new_columns = []
        whitespaces = [
            VerticalWS(ws=ws, top=ws.y1 == tb_area.y1, bottom=ws.y2 == tb_area.y2, position=id_area)
        ...
        for col in columns:
            ...
            matching_ws = [
                v_ws for v_ws in whitespaces if col.corresponds(v_ws=v_ws, char_length=char_length)
            ]
    ...
    dict_bounds = {
        k: {"y_min": table_areas[k].y1, "y_max": table_areas[k].y2} for k in range(len(table_areas))
    }
    reshaped_columns = []
    for col in columns:
        ...
        for v_ws in col.whitespaces:
            ...
            y_min = (
                dict_bounds.get(v_ws.position - 1, {}).get("y_max") or v_ws.y1
                if v_ws.top
                else v_ws.y1
            )
            y_max = (
                dict_bounds.get(v_ws.position + 1, {}).get("y_min") or v_ws.y2
                if v_ws.bottom
                else v_ws.y2
            )
            reshaped_v_ws = VerticalWS(
                ws=Whitespace(
                    cells=[
                        Cell(
                            x1=col.x1,
    ...
    max_height = max(col.height for col in reshaped_columns)
    return [col for col in reshaped_columns if col.height >= 0.66 * max_height]


def identify_columns(table_segment: TableSegment, char_length: float) -> ColumnGroup | None:
    """
    ...
    columns = get_columns_delimiters(table_segment=table_segment, char_length=char_length)
    ...
    if columns:
        ...
        column_group = ColumnGroup(
            ...
            elements=[
                el
                for el in table_segment.elements
                if el.x1 >= x1_del and el.x2 <= x2_del and el.y1 >= y1_del and el.y2 <= y2_del
            ],
        ...
        return (
            column_group
            if len(column_group.columns) >= 4 and len(column_group.elements) > 0
            else None
        )

...
def segment_image(
    ...
    """
    # Identify text mask
    text_thresh = identify_text_mask(
        thresh=thresh, lines=lines, char_length=char_length, existing_tables=existing_tables
    )

    # Identify image elements
    img_elements = get_image_elements(thresh=text_thresh, char_length=char_length)
    ...
    image_segment = ImageSegment(
        x1=0, y1=y_min, x2=thresh.shape[1], y2=y_max, elements=img_elements
    )

    col_segments = segment_image_columns(
        image_segment=image_segment, char_length=char_length, lines=lines
    )
    ...
    return [
        ...
        for table_segment in get_table_segments(
            segment=col_segment, char_length=char_length, median_line_sep=median_line_sep
        )
    ]

...
def get_table_areas(
    segment: ImageSegment, char_length: float, median_line_sep: float
) -> list[ImageSegment]:
    """
    ...
    if len(h_ws) == 0:
        h_ws = [
            Whitespace(
                cells=[
                    Cell(
                        x1=min([el.x1 for el in segment]),
                        x2=max([el.x2 for el in segment]),
                        y1=segment.y1,
                        y2=segment.y1,
                    )
                ]
    ...
    for idx, (up, down) in enumerate(pairwise(h_ws)):
        # Get the delimited area
        delimited_area = Cell(
            x1=max(min(up.x1, down.x1) - int(char_length), 0),
            y1=up.y2,
        ...
        area_elements = [
            el
            for el in segment
            if el.x1 >= delimited_area.x1
            and el.x2 <= delimited_area.x2
            and el.y1 >= delimited_area.y1
            and el.y2 <= delimited_area.y2
        ]
        seg_area = ImageSegment(
            x1=delimited_area.x1,
        ...
        if area_elements:
            ...
            v_ws = get_relevant_vertical_whitespaces(
                segment=seg_area, char_length=char_length, median_line_sep=median_line_sep, pct=0.66
            )
            ...
            if len(middle_ws) >= 1:
                ...
                left_ws = Whitespace(
                    cells=[
                        Cell(
                            x1=seg_area.x1,
                            y1=seg_area.y1,
                            x2=min([el.x1 for el in seg_area]),
                            y2=seg_area.y2,
                        )
                    ]
                )
                right_ws = Whitespace(
                    cells=[
                        Cell(
                            x1=max([el.x2 for el in seg_area]),
                            y1=seg_area.y1,
                            x2=seg_area.x2,
                            y2=seg_area.y2,
                        )
                ...
                v_ws = [
                    ...
                    and (
                        (len({ws.y1, ws.y2}.intersection({seg_area.y1, seg_area.y2})) > 0)
                        or (ws.height >= 0.66 * max([w.height for w in middle_ws]))
                    )
                ]

                seg_area.set_whitespaces(
                    whitespaces=sorted([*v_ws, left_ws, right_ws], key=lambda ws: ws.x1 + ws.x2)
                )
...
def coherent_table_areas(
    tb_area_1: ImageSegment, tb_area_2: ImageSegment, char_length: float, median_line_sep: float
) -> bool:
    """
    ...
    """
    if tb_area_1.position is None or tb_area_2.position is None:
        return False
    ...
    if tb_area_1.position < tb_area_2.position:
        ws_tb_1 = merge_consecutive_ws(
            [ws for ws in tb_area_1.whitespaces if ws.y2 == tb_area_1.y2]
        )
        ws_tb_2 = merge_consecutive_ws(
            [ws for ws in tb_area_2.whitespaces if ws.y1 == tb_area_2.y1]
        )
    else:
        ws_tb_1 = merge_consecutive_ws(
            [ws for ws in tb_area_1.whitespaces if ws.y1 == tb_area_1.y1]
        )
        ws_tb_2 = merge_consecutive_ws(
            [ws for ws in tb_area_2.whitespaces if ws.y2 == tb_area_2.y2]
        )
...
def table_segment_from_group(table_segment_group: list[ImageSegment]) -> ImageSegment:
    ...
    elements = [el for seg in table_segment_group for el in seg]
    whitespaces = [ws for seg in table_segment_group for ws in seg.whitespaces]

    # Create ImageSegment object
    return ImageSegment(
        x1=min([seg.x1 for seg in table_segment_group]),
...
def get_table_segments(
    ...
    """
    # Get table areas
    table_areas = get_table_areas(
        segment=segment, char_length=char_length, median_line_sep=median_line_sep
    )
    ...
    for tb_area in seq:
        ...
        if not coherent_table_areas(
            tb_area_1=prev_table,
            tb_area_2=tb_area,
    ...
    return [
        TableSegment(table_areas=tb_area_gp)
        for tb_area_gp in tb_areas_gps

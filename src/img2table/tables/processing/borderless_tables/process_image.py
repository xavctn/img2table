...
def prepare_image(img: np.ndarray, ocr_df: OCRDataframe) -> np.ndarray:
    """
    ...
    """
    # If an OCR is available, use image created from words
    if ocr_df is not None:
        processed_img = create_word_image(img=img,
                                          ocr_df=ocr_df)

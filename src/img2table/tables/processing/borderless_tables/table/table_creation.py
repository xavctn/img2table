...
def get_table(columns: ColumnGroup, row_delimiters: list[Cell], contours: list[Cell]) -> Table | None:
    """
    ...
    for col in columns.columns:
        seq = iter(
            sorted(
                [c for v_ws in col.whitespaces for c in v_ws.ws.cells], key=lambda c: c.y1 + c.y2
            )
        )

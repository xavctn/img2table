...
def merge_overlapping_contours(contours: list[Cell]) -> list[Cell]:
    ...
    df_cnt = pl.DataFrame(
        data=[
            {"id": idx, "x1": c.x1, "y1": c.y1, "x2": c.x2, "y2": c.y2, "area": c.area}
    ...
    df_cross = df_cross.with_columns(intersection.alias("intersection")).with_columns(
        (pl.col("intersection") / pl.col("area") >= 0.25).alias("overlaps")
    )
    ...
    df_overlap = (
        df_cross.filter(pl.col("overlaps"))
        .group_by(pl.col("id_right").alias("id"))
        .agg(
            pl.min("x1").alias("x1_overlap"),
    ...
    df_final = (
        df_cnt.join(deleted_contours, on="id", how="anti")
        .join(df_overlap, on="id", how="left")
        .select(
            [
                pl.min_horizontal("x1", "x1_overlap").alias("x1"),
...
def get_contours_cell(
    ...
    for c in cnts:
        ...
        x = x + cell.x1 - margin
        y = y + cell.y1 - margin
        contour_cell = Cell(x1=x, y1=y, x2=x + w, y2=y + h)
        list_cnts_cell.append(contour_cell)

    # Add contours to row
    return merge_contours(contours=list_cnts_cell, vertically=merge_vertically)

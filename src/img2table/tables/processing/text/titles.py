...
def get_title_tables(
    img: np.ndarray, tables: list[Table], ocr_df: OCRDataframe, margin: int = 5
) -> list[Table]:
    """
    ...
    for id_cl, cluster in enumerate(tb_cl):
        ...
        x_delimiters = [round((tb_1.x2 + tb_2.x1) / 2) for tb_1, tb_2 in pairwise(cluster)]
        x_delimiters = [
            max(10, round(cluster[0].x1 - 0.2 * cluster[0].width)),
            *x_delimiters,
            width - 10,
        ]
        x_delimiters = [
            *x_delimiters,
            min(width - 10, round(cluster[-1].x2 + 0.2 * cluster[-1].width)),
        ]
        x_bounds = [(del_1, del_2) for del_1, del_2 in pairwise(x_delimiters)]
        ...
        for id_tb, table in enumerate(cluster):
            ...
            cell_title = Cell(
                x1=x_bounds[id_tb][0], x2=x_bounds[id_tb][1], y1=y_bounds[0], y2=y_bounds[1]
            )
            contours = get_contours_cell(
                img=copy.deepcopy(img), cell=cell_title, margin=0, blur_size=5, kernel_size=9
            )

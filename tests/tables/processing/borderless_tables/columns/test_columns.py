...
def test_get_columns_delimiters() -> None:
    with Path("test_data/table_segment.json").open() as f:
        data = json.load(f)
    ...
    result = get_columns_delimiters(table_segment=table_segment, char_length=14)

    assert result == [
        Column(
            whitespaces=[
                VerticalWS(ws=Whitespace(cells=[Cell(x1=7, y1=0, x2=21, y2=544)])),
                VerticalWS(ws=Whitespace(cells=[Cell(x1=7, y1=496, x2=21, y2=660)])),
            ]
        ),
        Column(
            whitespaces=[
                VerticalWS(ws=Whitespace(cells=[Cell(x1=270, y1=69, x2=372, y2=544)])),
                VerticalWS(ws=Whitespace(cells=[Cell(x1=270, y1=496, x2=372, y2=626)])),
            ]
        ),
        Column(
            whitespaces=[
                VerticalWS(ws=Whitespace(cells=[Cell(x1=1659, y1=69, x2=1758, y2=544)])),
                VerticalWS(ws=Whitespace(cells=[Cell(x1=1659, y1=496, x2=1758, y2=626)])),
            ]
        ),
        Column(
            whitespaces=[
                VerticalWS(ws=Whitespace(cells=[Cell(x1=1845, y1=0, x2=1859, y2=544)])),
...
def test_identify_columns() -> None:
    with Path("test_data/table_segment.json").open() as f:
        data = json.load(f)
    ...
    result = identify_columns(table_segment=table_segment, char_length=14)

    with Path("test_data/delimiter_group.json").open() as f:
        data = json.load(f)
        expected = ColumnGroup(
            columns=[
                Column(whitespaces=[VerticalWS(ws=Whitespace(cells=[Cell(**d)])) for d in col])

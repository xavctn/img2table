...
from img2table.tables.processing.borderless_tables.layout.column_segments import (
    ...
    get_segments_from_columns,
    get_vertical_ws,
    identify_column_groups,
    identify_remaining_segments,
    is_column_section,
...
def test_identify_remaining_segments() -> None:
    searched_rectangle = Rectangle(x1=0, y1=0, x2=100, y2=100)
    existing_segments = [Cell(x1=0, y1=25, x2=35, y2=40), Cell(x1=59, y1=37, x2=78, y2=49)]

    result = identify_remaining_segments(
        searched_rectangle=searched_rectangle, existing_segments=existing_segments
    )

    expected = [
        Cell(x1=0, y1=49, x2=100, y2=100),
...
def test_get_vertical_ws() -> None:
    with Path("test_data/lines.json").open() as f:
        data = json.load(f)
    lines = [Line(**el) for el in data.get("h_lines") + data.get("v_lines")]

    with Path("test_data/elements.json").open() as f:
        elements = [Cell(**el) for el in json.load(f)]

    image_segment = ImageSegment(x1=0, y1=49, x2=768, y2=967, elements=elements)

    result = get_vertical_ws(image_segment=image_segment, char_length=5.04, lines=lines)

    expected = [
        Whitespace(cells=[Cell(x1=0, y1=106, x2=56, y2=1054)]),
...
def test_is_column_section() -> None:
    ws_gp_1 = [
        Cell(x1=0, x2=10, y1=100, y2=300),
...
def test_identify_column_groups() -> None:
    with Path("test_data/elements.json").open() as f:
        elements = [Cell(**el) for el in json.load(f)]
    ...
    result = identify_column_groups(image_segment=image_segment, vertical_ws=vertical_ws)
...
def test_get_column_group_segments() -> None:
    col_gp = [
        Cell(x1=0, x2=10, y1=0, y2=100),
    ...
    expected = [
        ImageSegment(x1=5, y1=30, x2=35, y2=100),
...
def test_get_segments_from_columns() -> None:
    with Path("test_data/elements.json").open() as f:
        elements = [Cell(**el) for el in json.load(f)]
    ...
    result = get_segments_from_columns(image_segment=image_segment, column_groups=col_gps)
...
def test_segment_image_columns() -> None:
    with Path("test_data/lines.json").open() as f:
        data = json.load(f)
    lines = [Line(**el) for el in data.get("h_lines") + data.get("v_lines")]
    ...
    image_segment = ImageSegment(x1=0, y1=0, x2=793, y2=1123, elements=elements)

    result = segment_image_columns(image_segment=image_segment, char_length=6.0, lines=lines)

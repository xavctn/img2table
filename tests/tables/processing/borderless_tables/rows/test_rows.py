...
from img2table.tables.processing.borderless_tables.rows import (
    correct_delimiter_width,
    filter_coherent_row_delimiters,
...
def test_identify_row_delimiters() -> None:
    ...
    with Path("test_data/h_whitespaces.json").open() as f:
        expected = [Cell(**c) for c in json.load(f)]

    assert result == expected


def test_filter_coherent_row_delimiters() -> None:
    row_delimiters = [
        Cell(x1=0, x2=100, y1=0, y2=0),
    ...
    column_group = ColumnGroup(
        columns=[
            ...
            Column(
                whitespaces=[VerticalWS(ws=Whitespace(cells=[Cell(x1=60, x2=60, y1=0, y2=20)]))]
            ),
            Column(
                whitespaces=[VerticalWS(ws=Whitespace(cells=[Cell(x1=100, x2=100, y1=0, y2=20)]))]
            ),
        ],
    ...
    result = filter_coherent_row_delimiters(
        row_delimiters=row_delimiters, column_group=column_group
    )

    expected = [Cell(x1=0, x2=100, y1=0, y2=0), Cell(x1=0, x2=100, y1=20, y2=20)]

    assert result == expected


def test_correct_delimiter_width() -> None:
    row_delimiters = [
        Cell(x1=0, x2=100, y1=0, y2=0),
        Cell(x1=0, x2=80, y1=10, y2=10),
    ...
    contours = [
        Cell(x1=23, x2=34, y1=12, y2=18),
        Cell(x1=86, x2=93, y1=2, y2=9),
        Cell(x1=3, x2=17, y1=18, y2=24),
    ]

    result = correct_delimiter_width(row_delimiters=row_delimiters, contours=contours)
...
def test_identify_delimiter_group_rows() -> None:
    with Path("test_data/delimiter_group.json").open() as f:
        data = json.load(f)
    column_group = ColumnGroup(
        columns=[
    ...
    result = identify_delimiter_group_rows(column_group=column_group, contours=contours)
